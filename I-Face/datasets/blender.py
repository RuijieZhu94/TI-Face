...
class BlenderDatasetBase():
    def setup(self, config, split):
        ...
        self.has_mask = self.config.apply_mask
        ...
        self.all_c2w, self.all_images, self.all_fg_masks, self.all_valids = [], [], [], []
        ...
        for i, frame in enumerate(meta['frames']):
            ...
            if self.split == "train":
                # img
                img_path = os.path.join(self.config.root_dir, f"{frame['file_path']}.jpg")
                img = Image.open(img_path)
                img = img.resize(self.img_wh, Image.BICUBIC)
                img = TF.to_tensor(img).permute(1, 2, 0) # (4, h, w) => (h, w, 4)                
                # valid mask
                valid_path = img_path.replace("images","masks").replace("jpg","npy")
                valid = np.load(valid_path)
                valid = torch.from_numpy(valid).squeeze() # (h, w)
                # fg mask
                fg_mask_path = img_path.replace("images","vit_mask").replace("jpg","png")
                fg_mask = Image.open(fg_mask_path)
                fg_mask = fg_mask.resize(self.img_wh, Image.BICUBIC)
                fg_mask = TF.to_tensor(fg_mask).permute(1, 2, 0).squeeze() # (h, w)
                # fg_mask[fg_mask > 0] = 1
            else:
                img = torch.ones([self.h, self.w, 3])
                valid = torch.ones([self.h, self.w])
                fg_mask = torch.ones([self.h, self.w])
            # valid = valid.view(-1) # (h*w) mask

            self.all_c2w.append(c2w)                          
            self.all_fg_masks.append(fg_mask) # (h, w)
        ...
        self.all_c2w, self.all_images, self.all_fg_masks, self.all_valids = \
            ...
            torch.stack(self.all_valids, dim=0).float().to(self.rank)
...
class BlenderDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage in [None, 'predict']:
            self.predict_dataset = BlenderDataset(self.config, self.config.test_split)
