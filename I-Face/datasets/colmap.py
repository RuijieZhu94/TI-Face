...
class ColmapDatasetBase():
    ...
    def setup(self, config, split):
        ...
        if not ColmapDatasetBase.initialized:
            ...
            all_c2w, all_images, all_fg_masks, all_valids = [], [], [], []
            ...
            for i, d in enumerate(imdata.values()):
                ...
                if self.split in ['train', 'val']:
                    ...
                    if has_mask:
                        mask_paths = [os.path.join(mask_dir, d.name.replace(".jpg", ".png")), os.path.join(mask_dir, d.name[3:])]
                    ...
                    if self.split == "train":
                        valid_path = img_path.replace("images","valid").replace(".jpg",".npy")
                        valid = np.load(valid_path)
                        valid = torch.from_numpy(valid) # (h, w, 1)
                    else:
                        valid = torch.ones_like(img[..., -1:])
        ...
        if self.split == 'test':
            ...
            self.all_valids = torch.ones((self.config.n_test_traj_steps, self.h, self.w), dtype=torch.uint8)
        else:
            self.all_images, self.all_fg_masks = torch.stack(self.all_images, dim=0).float(), torch.stack(self.all_fg_masks, dim=0).float()
            self.all_valids = torch.stack(self.all_valids, dim=0)
        """
        # for debug use
