...
for i in {0..37..2}; do
{
python launch.py --config configs/neus-blender_ilsh.yaml  --gpu 3 \
    --train dataset.scene=\'${exp_list[i]}\' tag=new \
    > ./exp/log_neus-new-${exp_list[i]}.txt 2>&1 &
python launch.py --config configs/neus-blender_ilsh.yaml  --gpu 4 \
    --train dataset.scene=\'${exp_list[i+1]}\' tag=new \
    > ./exp/log_neus-new-${exp_list[i+1]}.txt 2>&1 &
